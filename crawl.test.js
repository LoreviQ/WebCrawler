...
test('Converting the scheme and host to lowercase', () => {
    expect(normalizeURL('HTTP://User@Example.COM/Foo')).toBe('http://User@example.com/Foo')
})

test('Decoding percent-encoded triplets of unreserved characters', () => {
    expect(normalizeURL('http://example.com/%7Efoo')).toBe('http://example.com/~foo')
})

test('Removing dot-segments', () => {
    expect(normalizeURL('http://example.com/foo/./bar/baz/../qux')).toBe('http://example.com/foo/bar/qux')
})
...
test('Removing the fragment', () => {
    expect(normalizeURL('http://example.com/bar.html#section1')).toBe('http://example.com/bar.html')
})
...
test('Removing duplicate slashes', () => {
    expect(normalizeURL('http://example.com/foo//bar.html')).toBe('http://example.com/foo/bar.html')
})
